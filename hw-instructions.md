...
Clone your repo.  Your repo should be named hw-wordgame-on-rails-githubusername, where githubusername is your GitHub user name. 
...
```
rails new --skip-javascript --skip-test --skip-action-mailer --skip-action-mailbox --skip-action-text --skip-action-cable --skip-active-storage --skip-keeps --skip-spring --skip-sprockets --skip-turbolinks --database=postgresql wordgame
```
This creates your new app in a directory named `wordgame`.  All of the "skip" options are to turn off functionality that we don't need in this app.  The most important ones to turn off are:
...
Now that we have autogenerated a ton of stuff, let's go into that directory: `cd wordgame`
